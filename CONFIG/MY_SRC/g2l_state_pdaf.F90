...
!$AGRIF_DO_NOT_TREAT
  ...
  USE mod_parallel_pdaf, &
       ONLY: nldj_child, nldj_par, nldi_child, nldi_par, abort_parallel
  USE mod_agrif_pdaf, &
...
! *************************************
  ...
  ! Compute horizontal location in statevector.
  ...
  DO i = 1, dim_l
     IF( i <= SIZE(var2d_p_offset_par) ) THEN
        state_l(i) = state_p( var2d_p_offset_par(i) + loc_2d )
     ELSE
        ...
        c = MOD(a-1,dim_vert)
        state_l(i) = state_p( var3d_p_offset_par(b) + &
  ...
  ! Child grid
  child_grid: IF(domain_p > tot_dom_l_par) THEN
     ...
     DO i = 1, dim_l
        IF( i <= SIZE(var2d_p_offset_child) ) THEN
           state_l(i) = state_p( var2d_p_offset_child(i) + loc_2d )
        ELSE
           ! Now, fill the local state vector with 3D state variables.
           ! Each 3D state variable has dimension nx*ny*nz. We first break
           ! this into chunks of size nx*ny. Then we identify each chunk as a
           ...
           ! Counter for vertical coordinate.
           c = MOD(a-1,dim_vert)
           state_l(i) = state_p( var3d_p_offset_child(b) + &
                (mpi_subd_lon_child*mpi_subd_lat_child*(v_coord(c)-1)) + loc_2d )
        END IF
     END DO
