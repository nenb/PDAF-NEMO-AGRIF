...
!MMS:{
   CHARACTER(lc), PUBLIC :: cn_subbasins
!:MMS}
...
!MMS:{
    ! NAMELIST/namptr/ ln_diaptr, ln_subbas
      NAMELIST/namptr/ ln_diaptr, ln_subbas, cn_subbasins
!:MMS}
      !!----------------------------------------------------------------------
...
902   IF( ios /= 0 ) CALL ctl_nam ( ios , 'namptr in configuration namelist', lwp )
      ...
      IF(lwp) THEN                     ! Control print
         ...
         WRITE(numout,*) '      Subbasins file name                             cn_subbasins  = ', cn_subbasins   !MMS
      ENDIF
...
!MMS:{
    !       CALL iom_open( 'subbasins', inum,  ldstop = .FALSE.  )
            CALL iom_open( TRIM(cn_subbasins), inum,  ldstop = .FALSE.  )
!:MMS}
