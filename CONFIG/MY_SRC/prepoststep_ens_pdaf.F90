...
! *** local variables ***
  ...
  REAL :: rmserror_ssh_est                ! estimated RMS error
...
! **********************
  ...
  IF (mype_filter == 0) THEN
     IF (firsttime) THEN
        ...
        anastr = 'ini'
        CALL init_netcdf_asml(nit000, rdt, dim_p, filtertype, subtype, &
             dim_ens, forget, local_range, locweight, srange, &
             rms_obs, delt_obs, nitend, dim_lag)
...
!!$  END IF PE0_a
!!$
!!$!  VLADIMIR APPROACH WAS TO USE MPI_REDUCE:  
!!$  
!!$  CALL MPI_REDUCE( rmse_ssh, rmse_est_ssh_glo,1,MPI_DOUBLE_PRECISION,MPI_SUM,&
!!$       0, COMM_FILTER,ierr)
!!$
!!$  DEALLOCATE(variance_p)
!!$
!!$
!!$!  ISSUE IS WHEN DOMAIN CONTAINS HALO REGIONS, NEED TO BE CAREFUL NOT TO ADD
!!$!  THE HALO REGIONS TWICE IN THE SUM. ANOTHER
...
!!$
!!$  ! Vladimir alternative
!!$  IF (mype_filter==0) THEN
!!$     rmse_est_ssh_glo = sqrt(rmse_est_ssh_glo/real(jpiglo*jpjglo))
!!$  ENDIF
...
!!$  END IF notfirst
!!$
!!$  CALL write_netcdf_asml() !INSERT ARGUMENTS
!!$  CALL close_netcdf_asml()
