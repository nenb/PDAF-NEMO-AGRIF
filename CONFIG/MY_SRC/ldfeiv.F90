...
!MMS:{
      IF (ln_gmmask) THEN 
        DO jj = 2, jpjm1
          DO ji = fs_2, fs_jpim1   ! vector opt.
	  aeiw(ji,jj) = ABS( gmmask(ji,jj))  * aeiw(ji,jj)      ! weightening mask: 0.0 <= gmmask <= 1.0 
          END DO
        END DO
      ELSE
        ! Only decrease the coefficient in the tropics (20N-20S) 
...
!MMS:{ Upper bound should be used for all configurations, not only ORCA05!
!MMS  ! ORCA R05: Take the minimum between aeiw  and aeiv0
!MMS  IF( cp_cfg == "orca" .AND. jp_cfg == 05 ) THEN
         DO jj = 2, jpjm1
            DO ji = fs_2, fs_jpim1   ! vector opt.
...
!MMS:{
      CALL iom_put( "gmmask" , gmmask )   ! GM mask weight coefficient
!:MMS}
      !  
