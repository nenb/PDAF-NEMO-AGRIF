...
!!
MODULE mod_obs_ssh_par_pdafomi
!$AGRIF_DO_NOT_TREAT
  ...
  USE netcdf
  USE mod_agrif_pdaf, &
       ONLY: lowlim_ssh_par, upplim_ssh_par
 
  IMPLICIT NONE
  SAVE

  ! Variables which are inputs to the module (usually set in init_pdaf)
  LOGICAL :: assim_ssh_par    !< Whether to assimilate this data type
  REAL(pwp) :: rms_ssh_par    !< Observation error standard deviation (for constant errors)
  LOGICAL :: twin_exp_ssh_par ! Whether to perform an identical twin experiment
  REAL(pwp) :: noise_amp_ssh_par  ! Standard deviation for Gaussian noise in twin experiment


  ! One can declare further variables, e.g. for file names which can
  ! be use-included in init_pdaf() and initialized there.
  CHARACTER(len=200) :: file_ssh_par ! netcdf file holding observations
  REAL(pwp), ALLOCATABLE :: coord2grid_par(:,:)  ! Array to convert PE-local observation
...
! *********************************************
    ...
    IF (mype_filter==0) &
         WRITE (*,'(8x,a)') 'Assimilate observations - obs_ssh_par'

    ! Store whether to assimilate this observation type (used in routines below)
    IF (assim_ssh_par) thisobs%doassim = 1
...
! **********************************
    ...
    ! Format of ndastp_par is YYYYMMDD
    step_count=MOD(ndastp_par,100)
    IF(mype_filter == 0) WRITE(*,'(/9x, a, 2i)') &
         'mod_obs_ssh_par current date:', ndastp_par
    ...
    s = 1
    stat(s) = NF90_OPEN(file_ssh_par, NF90_NOWRITE, ncid_in)
    s = s + 1

    DO j = 1, s-1
       IF (stat(s) /= NF90_NOERR) THEN
          WRITE(*,'(/9x, a, 3x, a)') &
               'ERROR: NetCDF error in opening obs file:', file_ssh_par
          CALL abort_parallel()
       END IF
    END DO
    ...
    IF ( (MAXVAL(lon_ssh_obs) >= 360.0_pwp) .OR. &
       ...
       WRITE(*,*) 'ERROR: ssh_par longitude has non-standard format.'
       CALL abort_parallel()
    END IF
    ...
    DO j = 1, s - 1
       IF (stat(j) .NE. NF90_NOERR) THEN
          WRITE(*,'(/9x, a, 3x, a)') &
               'NetCDF error in closing obs file:', file_ssh_par
          CALL abort_parallel()
       END IF
    END DO
...
! *********************************
    ...
    IF (firsttime) THEN
       CALL obs_in_grid_lcl(lon_ssh_obs, lat_ssh_obs, &
            mpi_subd_lon_par, mpi_subd_lat_par, coord2grid_par)
       firsttime = .FALSE.
    ELSE
       ...
       IF (mype_filter ==0) THEN
          WRITE(*,'(/9x, a)') 'WARNING: Reusing coord2grid_par from first &
...
! ***********************************************************
    ...
    IF (coord2grid_par(1,1) > 0) THEN
       ! Use SIZE(coord2grid_par(:,1)) as upper limit for obs on local PE
       DO ind_obs = 1 , SIZE(coord2grid_par(:,1))
          i_obs = INT(coord2grid_par(ind_obs,1))
          j_obs = INT(coord2grid_par(ind_obs,2))
          ! Reject non-existent and unrealistic obs
          IF ( obs(i_obs, j_obs, 1) .GT. lowlim_ssh_par) THEN
             IF ( obs(i_obs, j_obs, 1) .LT. upplim_ssh_par) THEN
                i_pe = INT(coord2grid_par(ind_obs,3))
                j_pe = INT(coord2grid_par(ind_obs,4))
    ...
    IF(cnt_p == 0) THEN
       WRITE(*,'(/9x, a, i3, 3x, a, i3)') 'WARNING: No ssh_par observations on &
...
! ****************************************************************
    ...
    ivar_obs_p(:) = 1.0 / (rms_ssh_par*rms_ssh_par)
...
! ****************************************
    ...
    CALL PDAFomi_gather_obs_f(thisobs, dim_obs_p, obs_p, ivar_obs_p, ocoord_p, &
         thisobs%ncoord, local_range_par, dim_obs_f)
...
! ********************
  ...
  END SUBROUTINE init_dim_obs_f_ssh_par
...
!!
  SUBROUTINE obs_op_f_ssh_par(dim_p, dim_obs_f, state_p, ostate_f, offset_obs)
...
! ******************************************************
    ...
    IF (thisobs%doassim==1) THEN
       ! Great-circle interpolation obs operator for ssh_par
       CALL ssh_par_obs_op_gcirc(thisobs, 4, state_p, ostate_f, offset_obs)
    END IF

  END SUBROUTINE obs_op_f_ssh_par
...
!!
  SUBROUTINE init_dim_obs_l_ssh_par(domain_p, step, dim_obs_f, dim_obs_l, &
       off_obs_l, off_obs_f)
    ...
    USE mod_assimilation_pdaf, &   
         ONLY: coords_l, local_range_par, locweight, srange_par
...
!!$    ENDIF
    ...
    CALL PDAFomi_init_dim_obs_l(thisobs_l, thisobs, coords_l, &
         locweight, local_range_par, srange_par, &
         dim_obs_l, off_obs_l, off_obs_f)

  END SUBROUTINE init_dim_obs_l_ssh_par
...
!-------------------------------------------------------------------------------
  SUBROUTINE obs_in_grid_lcl (longitude, latitude, idim_pe, jdim_pe, obs_gp_lcl)
    ...
    USE mod_parallel_pdaf, &
         ONLY: nldj_par, nldi_par, mype_ens, COMM_filter, jpni_par, &
         jpnj_par, mype_filter
    USE mod_agrif_pdaf, &
...
! ************************************
    ...
    DO jj = 1, SIZE(latitude)
       DO ii = 1, SIZE(longitude)
          ...
          lat_obs = latitude(jj)
          ! If using AGRIF, ignore observations in AGRIF grid.
#if defined key_agrif
          ! See AGRIF_fixedgrids.in and coordinates.nc for AGRIF bounds.
          ! Don't ignore observations on North and East boundaries.
          IF(lat_obs > -49.7371_pwp .AND. lat_obs < 8.96321566_pwp) THEN
             ! Need to use .OR. to deal with discontinuity at 0.
             IF(lon_obs > 290.25_pwp .OR. lon_obs < 69.75_pwp) THEN
                CYCLE
             END IF
          END IF
#endif
...
!!$    IF (mype_filter == 0) THEN
!!$       OPEN(31, file = 'debug_arr_par.txt', status = 'new')
!!$       DO i =1,s
...
END MODULE mod_obs_ssh_par_pdafomi
