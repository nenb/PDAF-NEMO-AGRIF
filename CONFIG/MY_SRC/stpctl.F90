MODULE stpctl
   ...
   !! $Id: stpctl.F90 6204 2016-01-04 13:47:06Z cetlod $
   ...
   SUBROUTINE stp_ctl( kt, kindic )
      ...
      IF( kt == nit000 .AND. lwp ) THEN
         ...
         IF ( nn_components == jp_iam_sas ) THEN
            WRITE(clfname,'(a)') 'time_'//TRIM(ensstr)//'.step.sas'
         ELSE
            WRITE(clfname,'(a)') 'time_'//TRIM(ensstr)//'.step'
         ENDIF
#else
         ! open time.step file with special treatment for SAS
         IF ( nn_components == jp_iam_sas ) THEN
...
#if defined key_USE_PDAF
            ! Overwrite NEMO namelist for different members
            WRITE(fname,'(a)') 'solver_'//TRIM(ensstr)//'.stat'
            CALL ctl_opn( numsol, fname, 'REPLACE', 'FORMATTED', 'SEQUENTIAL', -1, numout, lwp, narea )
#else
