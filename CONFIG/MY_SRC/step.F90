...
!!gm   #  include "zdfddm_substitute.h90"
   ...
   !! $Id: step.F90 6405 2016-03-29 11:54:37Z pabouttier $
...
! FUS start : add time stamps for second and penuiltimate time step to check scalability
      IF( kstp == nit000+1 ) THEN
	CALL date_and_time(values = time)
	IF (lwp) Write(*,*) 'Time at second time step is: ',time(5),':',time(6),':',time(7) 
      ENDIF 
      IF( kstp == nitend-1 ) THEN
	CALL date_and_time(values = time)
	IF (lwp) Write(*,*) 'Time at penultimate time step is: ',time(5),':',time(6),':',time(7) 
      ENDIF 
! FUS end : add time stamps for second and penuiltimate time step to check scalability
