...
!!
MODULE mod_obs_fake_ssh_par_pdafomi
!$AGRIF_DO_NOT_TREAT
  ...
  LOGICAL :: assim_fake_ssh_par    !< Whether to assimilate this data type
  REAL(pwp) :: rms_fake_ssh_par    !< Observation error standard deviation (for constant errors)
  LOGICAL :: twin_exp_fake_ssh_par ! Whether to perform an identical twin experiment
  REAL(pwp) :: noise_amp_fake_ssh_par  ! Standard deviation for Gaussian noise in twin experiment

  ! One can declare further variables, e.g. for file names which can
  ! be use-included in init_pdaf() and initialized there.
  CHARACTER(len=200) :: file_fake_ssh_par ! netcdf file holding observations
...
!!
  SUBROUTINE init_dim_obs_f_fake_ssh_par(step, dim_obs_f)
    ...
    USE mod_statevector_pdaf, &
         ONLY: mpi_subd_lon_par, mpi_subd_lat_par
    USE mod_parallel_pdaf, &
         ONLY: nldj_par, nldi_par, COMM_filter, jpiglo_par, jpjglo_par, &
         nimpp_par, njmpp_par
    USE mod_agrif_pdaf, &
...
! *********************************************
    ...
    IF (mype_filter==0) &
         WRITE (*,'(8x,a)') 'Assimilate observations - obs_fake_ssh_par'

    ! Store whether to assimilate this observation type (used in routines below)
    IF (assim_fake_ssh_par) thisobs%doassim = 1
...
! **********************************
    ...
    IF(mype_filter == 0) WRITE(*,'(/9x, a, 2i)') &
         'mod_obs_fake_ssh_par current date:', ndastp_par
    ...
    s = 1
    stat(s) = NF90_OPEN(file_fake_ssh_par, NF90_NOWRITE, ncid_in)
    s = s + 1

    DO j = 1, s-1
       IF (stat(s) /= NF90_NOERR) THEN
          WRITE(*,'(/9x, a, 3x, a)') &
               'ERROR: NetCDF error in opening obs file:', file_fake_ssh_par
          CALL abort_parallel()
       END IF
    END DO
    ...
    ALLOCATE (obs(jpiglo_par, jpjglo_par, 1))
    ...
    DO j = 1, s-1
       IF (stat(j) .NE. NF90_NOERR) THEN
          WRITE(*, '(/9x, a, 3x, i1, 3x, a, 3x, a)') &
               ...
               ' from obs file:', file_fake_ssh_par
          CALL abort_parallel()
       END IF
    END DO
    ...
    DO j = 1, s - 1
       IF (stat(j) .NE. NF90_NOERR) THEN
          WRITE(*,'(/9x, a, 3x, a)') &
               'NetCDF error in closing obs file:', file_fake_ssh_par
          CALL abort_parallel()
       END IF
    END DO
...
! ***********************************************************
    ...
    DO j = 1, mpi_subd_lat_par
       DO i = 1, mpi_subd_lon_par
          ...
          i_obs = nimpp_par + i0 + i - 1
          j_obs = njmpp_par + j0 + j - 1
#if defined key_agrif
          ! See AGRIF_fixedgrids.in and coordinates.nc for AGRIF bounds.
          IF(j_obs > 133 .AND. j_obs < 269) THEN
             IF(i_obs > 434 .AND. i_obs < 716) THEN
                CYCLE
             END IF
          END IF
#endif
    ...
    IF(cnt_p == 0) THEN
       WRITE(*,'(/9x, a, i3, 3x, a, i3)') 'WARNING: No fake_ssh_par observations on &
          ...
          END IF
             ...
             IF ( obs(i_obs, j_obs, 1) .GT. lowlim_ssh_par) THEN
                IF ( obs(i_obs, j_obs, 1) .LT. upplim_ssh_par) THEN
                   ...
                   ocoord_p(1, cnt_p) = glamt_par(i+i0,j+j0)*rad_conv
                   ocoord_p(2, cnt_p) = gphit_par(i+i0,j+j0)*rad_conv
...
! ****************************************************************
    ...
    ivar_obs_p(:) = 1.0 / (rms_fake_ssh_par*rms_fake_ssh_par)
...
! ********************
  ...
  END SUBROUTINE init_dim_obs_f_fake_ssh_par
...
!!
  SUBROUTINE obs_op_f_fake_ssh_par(dim_p, dim_obs_f, state_p, ostate_f, offset_obs)
...
! ******************************************************
  ...
  END SUBROUTINE obs_op_f_fake_ssh_par
...
!!
  SUBROUTINE init_dim_obs_l_fake_ssh_par(domain_p, step, dim_obs_f, dim_obs_l, &
       off_obs_l, off_obs_f)
...
!!$    ENDIF
  ...
  END SUBROUTINE init_dim_obs_l_fake_ssh_par

!$AGRIF_END_DO_NOT_TREAT
END MODULE mod_obs_fake_ssh_par_pdafomi
