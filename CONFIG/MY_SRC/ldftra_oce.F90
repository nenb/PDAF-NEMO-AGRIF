...
!MMS:{  Spatial GM mask read in by routine ldfeiv_init
   LOGICAL      , PUBLIC      ::   ln_gmmask       = .FALSE.  !: Use GM mask (0.0 <= gmscale <= 1.0) if set to .true.
   CHARACTER(lc), PUBLIC      ::   cn_gmmask_file             !: Name of GM mask input file
   CHARACTER(lc), PUBLIC      ::   cn_gmmask_var              !: Variable name of GM mask
!:MMS}
...
!MMS:{
   REAL(wp), PUBLIC, ALLOCATABLE, SAVE, DIMENSION(:,:)   ::   gmmask               ! 2D mask ( 0 <= gmmask <= 1 )
!:MMS}
# elif defined key_traldf_c1d
