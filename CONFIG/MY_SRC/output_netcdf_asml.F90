...
! !INTERFACE:
  SUBROUTINE init_netcdf_asml(step, dt, dim, filtertype, subtype, &
       dim_ens, forget, local_range,locweight, srange, rms_obs, &
       delt_obs, total_steps, dim_lag)
...
! Initialize file and write global atributes
    ...
    attstr  = 'Assimilation into NEMO-AGRIF model'
...
! Write variables
    ...
    writetimedep: IF (dowrite .OR. calltype=='ini') THEN
       IF (write_states) THEN
          ...
          cnt2(1) = dim
          cnt2(2) = 1
          stat(s) = NF90_PUT_VAR(fileid, Id_state, state, pos2, cnt2)
          s = s + 1
       END IF
       IF (write_ens) THEN
          pos3(1) = 1
          ...
          cnt3(2) = dim_ens
          cnt3(3) = 1
          stat(s) = NF90_PUT_VAR(fileid, Id_ens, ens, pos3, cnt3)
          s = s + 1
       END IF
