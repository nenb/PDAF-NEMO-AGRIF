...
!! $Id: agrif_user.F90 6204 2016-01-04 13:47:06Z cetlod $
...
!FUS start: use nest jpn[i&|j] for LPE
      CHARACTER(len=20) :: namelistname
      NAMELIST/nammpp/ cn_mpi_send, nn_buffer, jpni, jpnj, jpnij, ln_nnogather
      IF( Agrif_Nbstepint() == 0 ) THEN
      namelistname='namelist_ref'
      numnam_ref=Agrif_Get_Unit()
      IF (.NOT. Agrif_Root()) namelistname=TRIM(Agrif_CFixed())//'_namelist_ref'
      OPEN(numnam_ref,file=namelistname,status='OLD',form='formatted')
      READ (numnam_ref,nammpp)
      CLOSE(numnam_ref)
      ENDIF
...
!FUS end: use nest jpn[i&|j] for LPE
