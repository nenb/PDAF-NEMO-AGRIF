...
# endif
      ...
      ENDIF
      !
! FUS start: fix to provide timing for base and nest
#if defined key_agrif
...
!FUS start: do not open and read namelist_ref - conflicting with agrif_user to use LPE
...
numnam_ref=numnam_cfg	! set numnam_ref to numnam_cfg to provide the right unit for other routines 
			! this only works if namelist_ref = neamelist_cfg!
!FUS start: do not open and read namelist_ref - conflicting with agrif_user to use LPE
...
! FUS start: enable nemo_partition for child
!         IF( Agrif_Root() ) CALL nemo_partition(mppsize)
         CALL nemo_partition(mppsize)
! FUS end: enable nemo_partition for child
#else
         jpni  = 1
         jpnj  = 1
         jpnij = jpni*jpnj
...
! FUS end: enable decomposition for child
!      IF( Agrif_Root() ) THEN
#if defined key_nemocice_decomp
...
!FUS start: numnam_ref is not used
!      IF( numnam_ref      /= -1 )   CLOSE( numnam_ref      )   ! oce reference namelist
!FUS end: numnam_ref is not used
