...
!MMS:{
   REAL(wp)        ::   rn_sssr_ds =   0.5      ! >0. maximum salinity difference for erp term [psu]
   REAL(wp)        ::   zsdif                   ! temporary salinity difference for erp computation
   INTEGER, PUBLIC ::   nn_sssr_ice =   1       ! damping under ice flag 
                                                ! (  0 : no damping ; 
                                                !    1: damping as ocean ; 
                                                !   >1 : enhanced damping (x icedmp) )
!:MMS}
...
!MMS:{      
      REAL(wp),DIMENSION(jpi,jpj)  ::   zcoefice   !MMS   ! local multiplicative coefficient for SSS damping under ice
!:MMS}
      !!----------------------------------------------------------------------
      !
...
!MMS:{      !  Reintroduce DRAKKAR modification for damping under ice (formerly nn_icedmp, now nn_sssr_ice) 
            !  The variable zcoefice containes the damping coefficient under ice depending on the ice fraction fr_i
               ...
               DO jj = 1, jpj
                  DO ji = 1, jpi
                     SELECT CASE ( nn_sssr_ice )
                       CASE ( 0 )    ;  zcoefice(ji,jj) = 1. - fr_i(ji,jj)              ! no damping under ice
                       CASE ( 1 )    ;  zcoefice(ji,jj) = 1.                            ! damping without taking ice into account
                       CASE  DEFAULT ;  zcoefice(ji,jj) = 1. +(nn_sssr_ice-1)*fr_i(ji,jj) ! reinforced damping (x nn_icedmp) under ice )
                     END SELECT
                  END DO
               END DO
            ENDIF
...
!                       &        * ( zcoefice(ji,jj) )         &      !MMS     DRAKKAR Modification for nn_sssr_ice 
!                       &        * ( sss_m(ji,jj) - sf_sss(1)%fnow(ji,jj,1) ) 
!:MMS}
                     zsdif = ( sss_m(ji,jj) - sf_sss(1)%fnow(ji,jj,1) )
                     IF ( rn_sssr_ds /= 0. )   zsdif = SIGN( 1., zsdif ) * MIN( rn_sssr_ds, ABS(zsdif))
                     zerp = zsrp * ( 1. - 2.*rnfmsk(ji,jj) )   &      ! No damping in vicinity of river mouths
                        &        * ( zcoefice(ji,jj) )         &      !MMS     DRAKKAR Modification for nn_sssr_ice 
                        &        * zsdif 
...
!     NAMELIST/namsbc_ssr/ cn_dir, nn_sstr, nn_sssr, rn_dqdt, rn_deds, sn_sst, sn_sss, ln_sssr_bnd, rn_sssr_bnd
      NAMELIST/namsbc_ssr/ cn_dir, nn_sstr, nn_sssr, rn_dqdt, rn_deds, sn_sst, sn_sss, ln_sssr_bnd, &
		 & rn_sssr_bnd, rn_sssr_ds, nn_sssr_ice
!:MMS}
      INTEGER     ::  ios
      !!----------------------------------------------------------------------
      !
