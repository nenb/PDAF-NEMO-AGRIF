...
!:MMS}
      ...
      NAMELIST/namtra_ldf/ ln_traldf_lap  , ln_traldf_bilap,                  &
         ...
         &                 rn_aht_m       ,                                   &
         !MMS:{ NEW PARAMETERS: cn_gnmask: filename, cn_gmmskvar: Variable name
         &                 ln_gmmask      , cn_gmmask_file , cn_gmmask_var
!:MMS}
      !!----------------------------------------------------------------------
...
!MMS:{
         WRITE(numout,*) '      Horizontal GM mask            ln_gmmask       = ', ln_gmmask
         WRITE(numout,*) '      Filename for GM mask          cn_gmmask_file  = ', cn_gmmask_file
         WRITE(numout,*) '      Variable name for GM mask     cn_gmmask_var   = ', cn_gmmask_var
!:MMS}
         WRITE(numout,*)
      ENDIF
...
!MMS:{
      ...
      IF( ln_gmmask ) THEN
        IF(lwp) THEN
          WRITE(numout,*)
          WRITE(numout,*) 'ldf_eiv_init : read GM mask'
        ENDIF

        ALLOCATE( gmmask(jpi,jpj ) , STAT=ierr )
        IF( ierr /= 0 )   CALL ctl_warn('ldf_tra_init: failed to allocate GM mask array')
        
        CALL iom_open( TRIM(cn_gmmask_file), inum )
        CALL iom_get( inum, jpdom_data, TRIM(cn_gmmask_var), gmmask )
        CALL iom_close( inum )
      ENDIF
#endif
!:MMS}
