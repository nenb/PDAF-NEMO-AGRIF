...
!MMS:{ 
   LOGICAL :: ln_totalprecip = .FALSE. !: logical flag if rain = 'total precip' or just 'liquid precip'
!:MMS}
...
#  include "vectopt_loop_substitute.h90"
   ...
   !! $Id: sbcblk_core.F90 6399 2016-03-22 17:17:23Z clem $
...
!          &                  sn_tdif, rn_zqt,  rn_zu
      NAMELIST/namsbc_core/ cn_dir , ln_taudif, rn_pfac, rn_efac, rn_vfac,  &
         &                  sn_wndi, sn_wndj, sn_humi  , sn_qsr ,           &
         &                  sn_qlw , sn_tair, sn_prec  , sn_snow,           &
         &                  sn_tdif, rn_zqt,  rn_zu    , ln_totalprecip
!:MMS}
...
!MMS:{ CORE forcing fields do not provide total precipitation but separate rain and snow (ln_totalprecip=.false.):
      IF ( .NOT. ln_totalprecip ) THEN
          sf(jp_prec)%fnow(:,:,1) = sf(jp_prec)%fnow(:,:,1) + sf(jp_snow)%fnow(:,:,1)
      ENDIF
!:MMS}
